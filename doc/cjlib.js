...
var cjs = function() {
  ...
  g.data = {};
  g.item = {};
  g.collectionUrl = 'http://api.fnarg.net/tsunamis/';
  ...
  function loadList(href) {
    ...
    if (ajax) {
      ...
      /* Accept added by gav to keep h5v happy */
      ajax.setRequestHeader('Accept', g.contentType);
      ajax.send(null);
      if (ajax.status === 200) {
        g.data = JSON.parse(ajax.responseText);
      } else if(ajax.status===415){
        e = JSON.parse(ajax.responseText);
        alert(
          e.collection.error.code+ "\n"+
          e.collection.error.title+ "\n"+
          e.collection.error.message
        );
      }
    }
  }
  ...
  function processItemLink(item, editable, x) {
    ...
    if (item) {
      ...
      a.href = item.href;
      a.title = 'item-link';
      a.appendChild(document.createTextNode('Tsunami #' + x));
      if (edit === true) {
        a.onclick = function() {
          showItem(item.href);
          return false;
  ...
  function templateLink() {
    ...
    a.appendChild(document.createTextNode('Upload Tsunami'));
    p = document.createElement('p');
...
window.onload = function() {
  ...
  c = cjs();
  c.g.collectionUrl = 'http://api.fnarg.net/tsunamis/';
